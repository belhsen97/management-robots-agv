...
class RobotAVG:
    ...
    def __enter__(self):
        ...
        self.publish(publish_topic,msg)
        return self
    ...
    def publish(self,topic,msg):
           ...
           status = result[0]
           if status == 0:
              print(f"Sent data for {self.robot_data['name'] } to topic {topic}")
           else:
              print(f"Failed to send data for {self.robot_data['name'] } to topic {topic}")
              self.msg_count += 1
